...
export class SensorRuleBlocklyComponent implements OnInit, AfterViewInit {
  ...
  availableFields = [["sensor_type", "sensor_type"], ["ip_src_addr", "ip_src_addr"], ["ip_dst_addr", "ip_dst_addr"]];

  addAvailableFieldsBlock(fields) {
    Blockly.Blocks['available_fields'] = {
      init: function() {
        this.appendDummyInput()
            .appendField(new Blockly.FieldDropdown(fields), "FIELD_NAME");
  ...
  onAdd(): void {
    ...
    this.availableFields.push([fieldName, fieldName]);
    this.addAvailableFieldsBlock(this.availableFields);
  }
